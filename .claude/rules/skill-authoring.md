...
`docs/sandbox-tls.md`「スキルごとの判定値」節の 5 判定値（`不要` / `一部要` / `要（本スキルは read-only）` /
`要（本スキルは主に API 経由）` / `要`）のいずれかを判定し、判定値に応じた次の雛形文を使う。判定手順・
全スキル一覧・実測記録は `docs/sandbox-tls.md` を正典とし、`skill-authoring.md` 側に一覧を二重管理しない。
