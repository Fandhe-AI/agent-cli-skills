...
**行番号では指定しない。** 本ファイルを編集するたびに行番号は動くため、行番号ベースの手順は
編集の翌日には誤りになる（実際、この節は一度その誤りを持ち込んだ）。置換対象は**内容**で
特定する。

置換対象は「導入する wrapper（共通テンプレート）」節の ```yaml コードブロック**の中に限る**。
そのブロック内に含まれる実プレースホルダは次の 3 箇所だけである。

| プレースホルダ | ブロック内の出現箇所 | 該当行の役割 |
|---------------|-------------------|-------------|
| `<YYYY-MM-DD>` | 1 箇所 | `# 参照 SHA の検証記録（<YYYY-MM-DD> 実測）:` |
| `<SHA>` | 2 箇所 | `# - 到達性: gh api ... compare/main...<SHA>` と `uses: ...@<SHA> # main` |

**「テンプレートの機械検証」節に現れる `0bd2cf930f5c78732abd539dfd468aa8c1363e15` は置換対象では
ない。** これはプレースホルダではなく、検証実行時点で得た**実測値そのもの**（既に解決済みの
具体的な SHA）を記録したスナップショットである。書き換えると「いつ・どの SHA に対して検証したか」
の記録が失われる。同節は `<SHA>` を含まないため、コードブロック内に限定して文字列置換する限り
誤って触れることはない。
...
4. テンプレートのコードブロックだけを別ファイルへ抜き出し（例:
   `awk '/^```yaml$/{f=1;next} /^```$/{if(f)exit} f' docs/update-external-rollout.md > wrapper.yml`）、
   そのファイルに対して `<SHA>` → `${SHA}`、`<YYYY-MM-DD>` → `${DATE}` を**文字列置換**する。
   抜き出したファイルには「テンプレートの機械検証」節が含まれないため、スナップショットの
   SHA を誤って書き換える経路が構造的に無い。置換後は
   `grep -nE '<SHA>|<YYYY-MM-DD>' wrapper.yml` が 0 件であることを確認する。
...
run [32088779994](https://github.com/Fandhe-AI/agent-cli-skills/actions/runs/32088779994) を
`workflow_dispatch` で実行し、新レポート
[#357](https://github.com/Fandhe-AI/agent-cli-skills/issues/357) が生成された。

```
- 上流 main SHA: `db80e6256d4630c87478af4d4e50c250cb9655c0`
- 検査対象リポジトリ: 119 件（非アーカイブ全件）
- 乖離: 24 件（下流 24 / 上流マーカー 0）
```

**`SYNC-CI-ABSENT` の出現は 0 件**（レポート本文に当該文字列が 1 件も無い）。対象 5 リポは
「最新 pin の wrapper (5 件)」へ分類された。前回レポート #341 は乖離 27 件・うち
`SYNC-CI-ABSENT` 5 件だったため、この 5 件が解消している。

`UNKNOWN` が 0 件 → 20 件へ増えているのは、すべて軸 5（schedule 生存確認）の
`workflow メタデータ取得が HTTP 403` であり、本導入とは独立した権限起因の検査不能である
（既存の wrapper 保有リポも同じ 403 を返す）。軸 1〜3 の判定には影響しない。

### 残作業

なし。イシュー #342 の受け入れ条件はすべて充足した。
