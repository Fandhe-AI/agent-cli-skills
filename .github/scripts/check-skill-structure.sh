...
# 追加依存なし (bash + grep + awk) で ubuntu-latest 上に即実行できる。
# 検証内容:
#   1. 各 skills/<name>/SKILL.md が frontmatter (--- 始まり) を持つ
#   2. frontmatter に name / description キーがある
#   3. name の値がディレクトリ名と一致する
#   4. .claude/skills/<name> から skills/<name> への symlink がリンク切れでない
