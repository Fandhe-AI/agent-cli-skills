...
skills/                               -- スキル本体（各ディレクトリに SKILL.md）
  ...
  implement-issue-tree/
    ...
    tests/                            -- node:test 回帰テスト（G0 ゲート・merge ループ・依存再判定・CONFLICTING→base 取り込み dispatch・Workflow 起動可否・Review 比較基準・state 書込みフォールバック）、lib/ に起動可否契約の実装
  implement-review/
  implement-review-pr/
  init-claude/
  update-docs/
  update-issue-tree/
    scripts/                          -- reassign-sub-issue.sh（closed 親下の残置 open issue 付け替え・孤児再配置。DELETE→POST を単一プロセスで冪等に実行。cross-repository sub-issue は非対応で exit 2）、reassign-lib.sh（Step 3/4 の呼び出し側共有ヘルパー。計画配列の存在・型検証と終了ステータス解釈。両フェンスが自己完結的に source する）
...
.claude/
  ...
  rules/
    ...
    ruleset-policy.md                 -- ブランチ ruleset 方針（strict 必ず false・required check の App 束縛・一括更新後の 3 軸検証）
    skill-vendoring-layout.md         -- downstream のスキル配置方針（.claude/skills は symlink / 実ディレクトリどちらでも可・配置差を理由に構成変更しない）
