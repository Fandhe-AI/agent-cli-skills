...
計画作成・実装・テスト実行は sandbox 環境で実行できる（ワークスペース内書き込みのみで、ネットワーク越しの操作を行わない）。`gh issue view` によるイシュー取得と、コミット後の `git push` のみネットワークを要する任意ステップであり、該当コマンド単位で sandbox 無効にして実行する（`docs/sandbox-tls.md` 参照）。
