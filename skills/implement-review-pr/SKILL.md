...
このスキルはネットワーク越しの GitHub 操作（`gh pr view` / `gh pr checks` 等）を必須とする。判定は「要（既定フローは read-only、任意ステップで書き込みあり）」（`docs/sandbox-tls.md` 参照）: 既定フローはすべて読み取りで書き込みを行わないが、`gh pr review` によるレビュー投稿を選択した場合は書き込みを伴う。該当コマンドはコマンド単位で sandbox 無効にして実行する。`gh pr review` を実行する場合も同様にコマンド単位で sandbox 無効にする。ネットワーク遮断を解除できない環境では実行できない。
