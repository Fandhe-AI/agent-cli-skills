...
このスキルはネットワーク越しの GitHub 操作（`gh pr view` / `gh pr checks` 等）を必須とする。既定フローはすべて読み取りで書き込みを行わない。該当コマンドはコマンド単位で sandbox 無効にして実行する（`docs/sandbox-tls.md` 参照）。`gh pr review` によるレビュー投稿は任意ステップであり、実行する場合も同様にコマンド単位で sandbox 無効にする。ネットワーク遮断を解除できない環境では実行できない。
