    ...
    "PreToolUse": [
      {
        "type": "command",
        "matcher": "Bash",
        "command": "CMD=$(echo \"$CLAUDE_TOOL_INPUT\" | python3 -c \"import sys,json; d=json.load(sys.stdin); print(d.get('command',''))\" 2>/dev/null || true); if echo \"$CMD\" | grep -qE 'rm\\s+-rf\\s+\\.claude'; then echo '[BLOCKED] .claude への rm -rf は禁止されています'; exit 1; fi"
      }
    ]
  }
}
...
- `exit 1` を返すと Claude はツール実行を中断する
