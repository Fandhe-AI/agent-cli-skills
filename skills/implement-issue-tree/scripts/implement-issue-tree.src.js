...
// このマーカーを必ず置くことで、削除側（optinRecordUpdateInstructions・prCreatePrompt 手順 1c）
// は「見出しがちょうど 1 件・本文の最終行がこのマーカーと完全一致・見出しと終端マーカーの間が
// 既知の行パターン（空行・OPTIN_RECORD_MARKER_PREFIX 行・箇条書き行）のみ」の 3 条件をすべて
// 実測できた場合に限って削除してよい。1 つでも満たさなければ削除せずそのまま次の追記へ進む
// （削除しないことは常に安全側であり、この判定の失敗は処理を中断しない）。
...
function optinRecordUpdateInstructions(item, impl, stepNo) {
  ...
  return [
    ...
    // 本文を道連れに失う。PR #504 codex P0）。そのため開始見出し・終端マーカー
    // （OPTIN_RECORD_END_MARKER）の両方で機械生成区間を本文の末尾として一意に識別できた場合
    // のみ削除し、識別できなければ削除せずそのまま手順 d の追記へ進む（削除しないことは常に
    // 安全側であり、この判定に失敗しても処理を中断しない）。
...
function prCreatePrompt(item, impl, outOfScope) {
  ...
  return [
    ...
    // 本文を道連れに失う。PR #504 codex P0）。そのため開始見出し・終端マーカー
    // （OPTIN_RECORD_END_MARKER）の両方で機械生成区間を本文の末尾として一意に識別できた場合
    // のみ削除し、識別できなければ削除せず後続の追記へ進む（削除しないことは常に安全側）。
    ...(optinRecordSection
      ? [
