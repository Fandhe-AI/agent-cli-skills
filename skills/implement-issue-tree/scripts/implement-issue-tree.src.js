...
// シェル手順）は「見出し行の最後の出現位置より後にこのマーカーが現れる（または本文末尾に達する
// まで既知の行パターンのみが続く）・その間の各行が既知の行パターン（空行・
// OPTIN_RECORD_MARKER_PREFIX 行・箇条書き行）のみ」を実測できた場合に限って削除してよい
// （v2: PR #504 レビューで、旧 v1 の「本文の最終行がこのマーカーと完全一致」という前提が
// Cursor Bugbot 等の外部ツールが本文末尾へコメントを追記するケースで恒久的に成立しなくなる
// ことが判明。マーカー自体の探索位置を「本文の末尾」から「見出しより後の最初の一致」へ変更した
// ため、マーカーより後ろの追記内容を削除範囲に巻き込まずに済む）。実測できなければ削除せず
