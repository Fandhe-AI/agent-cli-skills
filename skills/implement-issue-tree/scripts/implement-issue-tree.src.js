...
// runImplement の blocked 理由文言（許可形式外の判定結果の案内）に埋め込む、opt-in テスト
// 宣言の許可形式の要約。OPTIN_TEST_RUNNERS・OPTIN_TEST_RUNNER_SUBCOMMANDS（args.optinTestCommands
// 側の起動時検証 validateOptinCommandForm が参照する唯一の真実源）から動的に組み立てることで、
// 許可ランナー・サブコマンド制限が変わってもこの案内文が追随し損ねることを構造的に防ぐ
// （文字列をハードコードすると、ランナー追加・削除のたびに手動同期が必要になり陳腐化する）。
// 以前は「承認一覧に無いか許可形式外」としか書いておらず、ランナーごとの第 2 トークン制限
// （例: cargo run は不許可）や `..`/`//` 拒否が実装者に伝わらなかった（Issue #502）。
...
// invalid な opt-in 宣言を持つノードへの警告ログ文言。実際に blocked 判定を行うのは
// runImplement（kind: 'implement' のみ、item.optinTestsInvalid を参照）であり、verify-close
// （子イシューを持つノード）は runVerifyClose を通り optinTestsInvalid を一切参照しないため
// blocked にはならない。hasChildren に応じて実挙動と一致する文言を返す（Issue #502: 以前は
// ノード種別を区別せず一律「実装は起動せず blocked で停止する」と表示しており、verify-close
// ノードでは実際には止まらないのに止まるかのような誤った警告になっていた）。
...
// 記録節の終端マーカー（PR #504 codex P0）。旧方式は「見出し行から本文末尾まで」を無条件に
// 削除しており、既存 PR 本文が非信頼データである以上「同名見出しから先はワークフロー所有」と
// 断定できなかった（ユーザーが説明・引用等で同じ見出しを書いていた場合、Closes 行・対象外節を
// 含む正当な本文を道連れに失う）。renderOptinRecordSection が生成する節の実際の最終行として
// このマーカーを必ず置くことで、削除側（optinRecordRemovalShellLines が両呼び出し元へ共有する
// シェル手順）は「見出し行の最後の出現位置より後にこのマーカーが現れる・その間の各行が既知の
// 行パターン（空行・OPTIN_RECORD_MARKER_PREFIX 行・箇条書き行）のみ」を実測できた場合に限って
// 削除してよい（v2: PR #504 レビューで、旧 v1 の「本文の最終行がこのマーカーと完全一致」という
// 前提が Cursor Bugbot 等の外部ツールが本文末尾へコメントを追記するケースで恒久的に成立しなく
// なることが判明。マーカー自体の探索位置を「本文の末尾」から「見出しより後の最初の一致」へ変更
// したため、マーカーより後ろの追記内容を削除範囲に巻き込まずに済む）。
//
// v3（PR #504 レビュー 2 巡目 codex P1）: v2 は終端マーカーが見つからない場合、既知の行パターン
// のみが続くことを条件に本文末尾までを削除する fallback を持っていた。しかし PR 本文は非信頼
// データであり、ユーザーが同じ見出し・同じ箇条書き書式（`- コマンド: ` 等）で説明・手動記録を
// 書ける以上、「行の見た目が既知パターンに一致する」ことは「ワークフローが生成した区間である」
// ことの証明にならない。終端マーカーという推測不能な境界が両端（見出しの後・区間の終わり）に
// 揃って初めて機械生成区間と実測できるため、v3 は fallback を廃止し、終端マーカーが見つからない
// 場合（終端マーカーを持たない旧書式の記録節を含む）は一切削除しない。実測できなければ削除せず
// そのまま次の追記へ進む（削除しないことは常に安全側であり、この判定の失敗は処理を中断しない。
// 旧い記録節が残ったまま次の手順で新しい節が追記され、見出しが一時的に重複し得るが、正当な本文
// を巻き込んで削除するより優先する）。
// v5（PR #504 codex P0 4 巡目）: v4 の終端マーカーは固定・公開文字列
// （'<!-- /opt-in-test-record -->'）のままだった。bodyNorm===exp の完全一致ゲートと合わせても、
// マーカー自体は「PR 本文を書ける利用者なら誰でも書ける」推測可能な値のため、公開情報
// （HEAD sha・宣言済みコマンド・結果 enum・既定の補足文言）だけから exp を再現できれば、
// マーカーと本文の両方を攻撃者が単独で偽造できてしまう。v5 は終端マーカーへ
// boundaryNonce（実行時に /dev/urandom 由来の seed で鍵付けした値。PR 本文はもちろん
// ソースコードにも現れない非公開値）由来の per-round nonce を埋め込み、削除条件を
// 「(a) 本文が host 側の信頼済み実測 exp と完全一致」かつ「(b) 終端マーカーが host が
// その回に実際に発行した nonce と一致」の AND にする。(b) だけでは不十分な点に注意:
// nonce は一度 PR 本文へ書かれた時点で公開情報になるため、次ラウンドで攻撃者がその
// nonce をコピーして偽の見出し＋本文を包む終端マーカーとして使い回せば、(b) 単独では
// 正当な本文（Closes 行・対象外節等）を巻き込んで削除させられる（v4 からの後退）。
// (a) の完全一致ゲートを外さず、nonce を「削除してよい区間を一意に特定する追加の鍵」
// として重ねることで、攻撃者は exp（host 側実測）と nonce（host 側秘密）の両方を同時に
// 言い当てない限り削除条件を満たせない。
...
// 記録節の nonce を「item 番号 + ラウンド識別子（fix ラウンドの通し番号。PR 作成時点の初期
// 節には呼び出し元から固定の 'create' を渡す）」から boundaryNonce で導出する。同一ラウンドの
// 呼び出しは常に同じ nonce を再現する（resume でも同一）が、他ラウンド・他 issue の nonce とは
// 一致しない。boundaryNonceSeed が未初期化の場合は boundaryNonce 自体が例外を投げる
// （fail-closed。予測可能な nonce で境界を区切ることを避ける）。
...
// PR 本文から opt-in テスト記録節を安全に除去する共有シェル手順（optinRecordUpdateInstructions・
// prCreatePrompt の双方が呼ぶ。PR #504 codex P0 と cursor[bot] Medium 指摘 2 件の修正）。
// 従来この 2 箇所は同一ロジックを個別に保持しており、修正が片方にしか反映されずドリフトする
// リスクがあった（cursor[bot] 指摘の対象箇所そのもの）ため、1 関数へ集約する。
//
// v1 の欠陥（実測で判明。全て本関数の設計で解消）:
//   - 「本文の最終行が終端マーカーと完全一致」を削除の前提条件にしていたため、外部ツール
//     （Cursor Bugbot 等）が本文末尾へコメントを追記すると削除条件が永久に成立しなくなった。
//     本関数は「見出しより後で終端マーカーが最初に現れる行」を探す方式に変え、マーカーより
//     後ろの内容を削除範囲に含めない（保持する）。
//   - 「見出し行がちょうど 1 件」を要求していたため、この修正が入る前に作成された既存 PR
//     （終端マーカーを持たない旧書式の記録節が残っている）では削除が一度も成立せず、以降の
//     ラウンドで見出しが増え続けた（cursor[bot] Medium 指摘）。本関数は「見出しの最後の出現
//     行」を対象にすることで、増分だけを毎回除去できるようにする。
//   - CRLF 本文で tail -n 1 の完全一致比較が失敗していた（cursor[bot] Medium 指摘）。本関数は
//     比較前に各行の末尾 \r を取り除く。
//   - body=$(sed ...) / s=$(sed ...) の終了コードを確認しておらず、いずれかの異常終了時に
//     空文字のまま検証を通過し得た（codex P0）。本関数は各コマンドの終了コードを確認し、
//     削除の書き戻しは単一の sed 実行の終了コードで gate する（非 0 なら一時ファイルを破棄し
//     mv しない。"$f" 自体は最後まで一切変更されない）。
//
// v3 の追加修正（PR #504 レビュー 2 巡目。codex P1 + cursor[bot] Low）:
//   - 終端マーカーが見つからない場合に「本文末尾まで既知の行パターンのみ」を条件に削除する
//     fallback を廃止した（codex P1）。行の見た目（空行・箇条書き）は誰でも書けるため、
//     所有権の証明にならない。終端マーカーという推測不能な境界を実測できた場合のみ削除する。
//   - 削除の書き戻しをコマンド置換（$(...)）経由に変える（cursor[bot] Low）。旧 v1 は
//     kept prefix を `s=$(sed -n "1,$((L-1))p" "$f"); printf '%s' "$s"` の形で書き戻しており、
//     コマンド置換が末尾の改行をすべて剥がすため、削除直後の "$f" は常に「末尾に空行が残らない」
//     状態に揃っていた（printf '%s' は改行を足さない）。v2 は `sed "${L},${DEL_END}d" "$f" > "$g"`
//     で直接ファイルを書き換えていたため、削除範囲より前に残っていた空行がそのまま温存され、
//     追記側の `{ echo; echo; ... }`（見出し直前に空行 1 行を保証する定型手順）と組み合わさる
//     たびに空行が積み増しされた。v3 は sed 実行後の "$g" をコマンド置換で読み直してから
//     `printf '%s'` で "$f" へ書き戻し、v1 と同じ「末尾の空行を持ち越さない」性質を復元する
//     （削除範囲は変えず、書き戻し経路だけを直す）。
//
// v4（PR #504 codex P0 3 巡目）: v3 は終端マーカー（当時の固定文字列定数）
// の実測を削除条件にしたが、このマーカー自体はソース上に固定された公開文字列であり、PR 本文を
// 書けるユーザーなら誰でも同じ見出し・箇条書き・マーカーを「正当な説明」として書ける。その場合
// v3 は「行の見た目が既知パターンに一致する」を根拠に削除してしまい、見出しとマーカーの間へ
// ユーザーが挟んだ Closes 行・対象外節等の正当な本文を道連れに失う（所有権の証明になっていない）。
// v4 は「行の見た目」ではなく「直前に自分（ホスト側が保持する信頼済み state）が実際に書いた
// 内容とのバイト完全一致」を条件にする。呼び出し元（optinRecordUpdateInstructions）が
// optinRecordExpectedLines() でホスト側の lastFixOptin（post-push fix の実測 headSha/runs。
// PR 本文を経由しない、プロセス内メモリまたは状態ファイルから復元した値）から「直前に書いた
// はずの本文」を再構成できた場合に限り、その再構成値を expectedLines として渡す。本文が
// 未信頼データである以上、値そのものが本物かはこの関数からは判定できないが、expectedLines は
// PR 本文を一切経由せず host 側の実測だけから組み立てているため、攻撃者が PR 本文の記述だけで
// 一致させることは（sha・結果・補足の実測値を言い当てない限り）できない。expectedLines が
...
// optinRecordRemovalShellLines へ渡す期待値（削除してよい既存記録節の正確な内容）を、PR 本文を
// 一切経由しない信頼済み host state（lastFixOptin: post-push fix が実測した headSha/runs）から
// 再構成する（PR #504 codex P0 3 巡目）。optinRecordUpdateInstructions（手順 d）が書く本文と
// 完全に同じ行構成（マーカー行 → コマンド → 結果 → 補足、ブロック間を空行 1 行で区切る）を
// 再現する必要があり、両者がドリフトするとここでの一致判定が常に失敗して削除が起きなくなる
// （安全側だが、見出しが際限なく重複する）。書式を変える場合は両方を同時に直すこと。
// 再構成できない場合（宣言なし・fix 未実施・headSha 未確定・nonce 未確定・宣言コマンドと
// runs が不一致）は null を返し、呼び出し元は削除を行わない。nonce の妥当性検証（PR #504
// codex P0 4 巡目）: lastFixOptin.nonce は host が直前ラウンドの記録節を書いた際に実際に
// 発行した boundaryNonce 由来値でなければならない。存在しない・書式不正（旧バージョンで
// 永続化された state の再開時等）の場合は削除しない（headSha 不明時と同じ fail-closed）。
...
function restoreOptinFixState(saved, declaredOptinTests) {
  ...
  // nonce は boundaryNonce の「実行時に導出した値そのもの」を state.nonce としてそのまま復元する
  // （headSha と同じ扱いで、resume 後のプロセスが別 seed で再導出するわけではない。resume で
  // 別プロセスの新しい seed を使って計算し直すと、既に PR 本文へ書かれている旧プロセスの nonce
  // とは一致しなくなり削除が永久に働かなくなる。PR #504 codex P0 4 巡目）。書式が不正・欠落
  // （このフィールドが無い旧バージョンの永続化を含む）の場合は null にする（削除しないだけで
  // unbound には倒さない — 記録ゲート自体の合否判定には影響させない）。
...
// 参照）。
// itemNumber: nonce 導出の鍵材料（PR #504 codex P0 4 巡目）。この節は PR 作成時に 1 回だけ
// 書かれ、比較対象となる host 側実測（lastFixOptin）がまだ存在しないため
// optinRecordExpectedLines は常に null を返し、この節自体が削除対象に選ばれることはない
// （削除は post-push fix が書いた節にのみ働く）。それでも終端マーカーの書式を統一するため
// 固定のラウンド鍵 'create' で nonce を発行する。
function renderOptinRecordSection(itemNumber, commands) {
  ...
  // 終端マーカーを節の実際の最終行として必ず置く（optinRecordEndMarker のコメント参照）。
  // 削除側はこのマーカーが host 発行の nonce と完全一致する場合のみ削除候補として扱う。
...
// 束縛を追加 → Issue #502 で節ごと削除方式へ変更）。renderOptinRecordSection は prCreatePrompt
// 呼び出し時点（PR 作成時の 1 回のみ）でしか呼ばれておらず、fix はこの呼び出し内で push まで
// 完結するため host 側が結果を受け取ってから renderOptinRecordSection を呼ぶ機会がない。その
// ためマーカー書式（optinRecordMarkerLine）をエージェント自身に厳密に再現させ、マージ前ゲート
// （optinRecordVerifyPrompt の固定文字列 grep）が古い pass 記録のまま停止せず通過してしまう
// 陳腐化を防ぐ。item.optinTests が空なら [] を返す（宣言なしイシューでは fixPrompt の出力を
// 完全に不変に保つ。R3 と同じ方針）。
//
// 旧方式はマーカー行のみを grep -vF で除去していたため、「## opt-in テスト実行記録」見出しと
// - コマンド: / - 結果: / - 補足: の人間可読行は除去対象外のまま残り、fix ラウンドを重ねる
// たびに見出しが重複し、古い結果の記述が矛盾したまま PR 本文へ蓄積していた（Issue #502）。
// 新方式は見出し行から本文末尾までを sed で節ごと削除してから今回分を書き直す。この節は
// 常に本文の最後に追記される契約（prCreatePrompt・本関数のいずれも呼び出し元は末尾追記のみ）
// のため、削除が他の節を巻き込むことはない。
// lastFixOptin: 呼び出し元（runMergeLoop）が保持する「直前の post-push fix ラウンドで実際に
// PR 本文へ書いた内容」の host 側実測（PR 本文を経由しない）。削除対象の完全一致比較と、
// その節が発行時に持っていた nonce 付き終端マーカーの再現に使う（PR #504 codex P0 3/4 巡目。
// optinRecordExpectedLines 参照）。無い場合（初回ラウンド・fix 未実施・resume で復元不能）は
// 削除を一切行わない。
// writeNonce: 呼び出し元が「今回このラウンドが新たに書く記録節」用にあらかじめ発行した
// boundaryNonce 由来値（host 側のみが知る非公開値。エージェントへは文字列そのものを渡す
// だけで、鍵となる seed 自体は渡さない）。手順 d の終端マーカーに埋め込み、次ラウンドの
// 削除判定（optinRecordExpectedLines が lastFixOptin.nonce として参照する）と対になる。
function optinRecordUpdateInstructions(item, impl, stepNo, lastFixOptin, writeNonce) {
  ...
  if (!writeEndMarker) {
    // 呼び出し元（runMergeLoop）は commands が非空なら必ず optinRecordNonce で有効な nonce を
    // 発行してから本関数を呼ぶ契約。ここに到達して不正ということは呼び出し側の実装不備であり、
    // 予測可能・欠落した終端マーカーで記録節を書かせるより fail-closed で停止する方が安全。
  ...
  return [
    ...
    // 無条件の「見出し行から本文末尾まで削除」（sed '/^見出し/,$d'）は、既存 PR 本文が
    // 非信頼データである以上「同名見出しから先は必ずワークフロー所有」と断定できない
    // （ユーザーが説明・引用等で同じ見出しを書いていた場合、Closes 行・対象外節を含む正当な
    // 本文を道連れに失う。PR #504 codex P0）。削除ロジックは optinRecordRemovalShellLines に
    // 集約されている（同一ロジックを個別に保持すると修正がドリフトする。cursor[bot] 指摘）。
...
function prCreatePrompt(item, impl, outOfScope) {
  ...
  return [
    ...
    // opt-in テスト記録節の除去は、他のどの追記（Closes 行・対象外節）よりも前に行う
    // （Issue #502）。除去を後回しにすると、この回で新規に追記した Closes 行・対象外節が
    // 古い opt-in 見出しより後ろに来て一緒に削除されてしまう（再利用経路が繰り返されるたびに
    // 前回追記した対象外節が消える、というデータ損失。除去を最初に行えば、この後で追記する
    // 内容は常に「除去済みの末尾」に積まれるため巻き込まれない）。
    //
    // 無条件の「見出し行から本文末尾まで削除」（sed '/^見出し/,$d'）は、既存 PR 本文が
    // 非信頼データである以上「同名見出しから先は必ずワークフロー所有」と断定できない
    // （ユーザーが説明・引用等で同じ見出しを書いていた場合、Closes 行・対象外節を含む正当な
    // 本文を道連れに失う。PR #504 codex P0）。削除ロジックは optinRecordRemovalShellLines に
    // 集約されている（同一ロジックを個別に保持すると修正がドリフトする。cursor[bot] 指摘）。
    // このプロンプト（prCreatePrompt）は PR 作成前に呼ばれ、post-push fix が一度も走っていない
    // ため「直前にホスト側が実際に書いた内容」の信頼済み実測（lastFixOptin）が存在しない。
    // 比較対象を持たないまま終端マーカーの見た目だけで削除するとユーザーの正当な本文を巻き込み
    // 得る（PR #504 codex P0 3 巡目）ため、expectedLines を渡さず削除自体を行わない
    // （optinRecordRemovalShellLines(undefined) は no-op を返す）。
    ...(optinRecordSection
      ? [
    ...
    ...(outOfScopeItems.length
      ...
      : []),
    // opt-in テスト記録節の再追記（除去は上の除去ステップで完了済み）。見出し行はここで
    // 初めて "$f" へ現れるため、直前行と結合させない空行の作り方を明示する（Issue #502:
    // 単純な heredoc 連結だと "$f" の末尾が改行で終わっていない場合に直前行へ見出しが
    // 接着し、上の sed パターン・人間の可読性の双方を壊す）。
    ...(optinRecordSection
      ? [
...
// lastFixOptin: 呼び出し元（runMergeLoop）保持の直前 post-push fix ラウンドの host 側実測。
// pushAfterFix: true のときのみ optinRecordUpdateInstructions（手順 4b）へ渡し、削除対象の
// 完全一致比較・nonce 付き終端マーカーの再現に使う（PR #504 codex P0 3/4 巡目）。
// pushAfterFix: false（Review fix）では手順 4b 自体を呼ばないため未使用。
// optinWriteNonce: 呼び出し元がこのラウンド用にあらかじめ発行した boundaryNonce 由来値
// （PR #504 codex P0 4 巡目）。手順 4b の新規記録節の終端マーカーへ埋め込む。呼び出し元は
// item.optinTests が非空の場合、この呼び出しの前に optinRecordNonce(item.number, roundKey)
// で発行し、ラウンド完了後の lastFixOptin.nonce へ同じ値を格納すること（次ラウンドの削除が
// この値を再現できるようにするため）。
...
for (const n of tree.nodes) {
  ...
  // verify-close（子を持つ親ノード）は PR を作らないため記録ゲート・blocked 判定いずれの
  // 適用対象外（runVerifyClose は optinTestsInvalid を一切参照しない）。kind の確定は visit()
  // 後だが、ここでは byParent 構築前のため children の有無で代替判定する（Issue #502:
  // optinInvalidWarningLine の文言選択にもこの判定を使い、ノード種別ごとに実挙動と一致する
  // 警告を出す）。
...
async function runMergeLoop(item, impl, initialFixCount, initialWorktreePath, initialOutOfScopeLog = [], initialUnresolvedInfo = '', initialUnresolvedComments = [], initialOutOfScopeSeen = [], initialBaseMergeCount = 0, initialPushMergeable = '', initialFixOptin = null) {
  ...
  while (!merged && monitorsLeft > 0) {
    ...
    if (lastState === 'needs-fix' || lastState === 'unresolved-comments') {
      ...
      // このラウンドが新たに書く opt-in テスト記録節用の nonce（PR #504 codex P0 4 巡目）。
      // fixCount（増加前）+ 1 をラウンド識別子にする（ちょうどこのラウンドの通し番号と一致し、
      // 下の fixCount++ 後は同じ数値が「直前ラウンドの番号」として lastFixOptin.nonce へ
      // 引き継がれる）。宣言なしイシューでは optinRecordUpdateInstructions が commands.length
      // === 0 の早期 return で未使用のまま終わるため、常に発行してよい。
      ...
      if (Array.isArray(item.optinTests) && item.optinTests.length > 0) {
        ...
        if (f.pushed === true) {
          ...
          if (fixHeadSha) {
            // nonce: このラウンドが手順 4b で実際に書いたはずの記録節の終端マーカー nonce
            // （PR #504 codex P0 4 巡目）。f.pushed === true の間は step d が実行された前提
            // （既存の headSha 同様、実行の成否自体は自己申告に依存する fail-open 部分だが、
            // これは content 完全一致チェックが多層防御として補う）。次ラウンドの
            // optinRecordExpectedLines がこの値を lastFixOptin.nonce として参照し、削除対象の
            // 終端マーカーを再現する。
