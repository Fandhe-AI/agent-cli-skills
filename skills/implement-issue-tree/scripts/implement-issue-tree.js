...
function baseMergeInstruction(base) {
  return `merge 前に対象リポの commitlint 設定（commitlint.config.* / .commitlintrc* / package.json の commitlint フィールド）を読み、マージコミットの subject を <type>[(<scope>)]: base ブランチの変更を取り込む の形式で決める（type は type-enum が無ければ chore、あれば chore → build → ci → fix の順で最初に許可されるもの。scope は scope-empty が never の場合のみ scope-enum から最も近い値（判断できなければ先頭）を付け、それ以外は省略する）。git merge --no-edit -m "<subject>" origin/${base} を実行し、終了コードで 3 分岐する: (a) 0（Already up to date またはクリーンマージ）→ 次の手順へ進む。(b) 非 0 かつ git diff --name-only --diff-filter=U が非空 → コンフリクト。その場で解消を試みる（対象リポジトリの CLAUDE.md・rules を遵守し、解消したらテスト実行規約に従いビルド・lint・テストを通してから git commit --no-edit でコミットする — subject は MERGE_MSG に残る上記のものを使う）。解消に確信が持てない・解消不能な場合は git merge --abort し、push せず「base コンフリクト解消不能」を理由として返す。(c) 非 0 かつ U が無い（MERGE_HEAD が残る = commit-msg hook 拒否等）→ git merge --abort し、push せず「base merge コミット拒否: <hook 出力の要旨>」を理由として返す（fail-closed。--no-verify で強行しない。exit 1 を無視して push すると base 未取り込みの HEAD が push されゲートを迂回する）。`
}
