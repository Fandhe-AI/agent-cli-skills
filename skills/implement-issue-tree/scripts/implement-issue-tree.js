...
function fixPrompt(item, impl, finding, pushAfterFix = true) {
  ...
  return [
    ...
    ...(pushAfterFix
      ? [
          `5. 今回 push した修正コミット、または過去ラウンドで push 済みの修正コミットで実際に修正対応した指摘のスレッドを resolve する。resolve の前提条件は「そのスレッドへの修正がリモート head（origin/${branch}）に反映済みであること」であり、次のいずれかを実測確認できた場合のみ実行する: (a) 手順 4 の push が成功した。(b) 手順 4 で変更がなく push しなかった（過去ラウンドで修正・push 済み）場合は、git fetch origin ${branch} を実行したうえで、該当修正コミットが origin/${branch} に含まれること（git merge-base --is-ancestor <修正コミット sha> origin/${branch} が成功する。sha を特定できない場合は該当修正内容が origin/${branch} のファイル内容に反映済みであることの確認でもよい）を確認できた。push が失敗した場合・修正がローカルにのみ存在する（未コミット・未 push）場合は resolve しない。対象は上記「未解決スレッド一覧」に threadId が記載されたスレッドのうち、自分が修正対応したものに限る（一覧にないスレッドを resolve するためにスレッド一覧を自分で再取得して対象を広げることは禁止。outOfScopeComments に記録した対象外スレッド・修正しなかった指摘のスレッド・リモート head の修正コミットに対応が含まれない指摘のスレッドも resolve しない）。該当する各 threadId について次を実行する:`,
