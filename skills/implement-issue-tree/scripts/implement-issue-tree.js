...
// enum に依存しない機械的な完全一致比較に限定する。同じ理由で fmt/lint/build/test も
// 実行しない（PR #443 P0・PRRT_kwDORuXFg86cbSqr。詳細は手順 2）。
function baseMergePrompt(item, impl, expectedRepo) {
  ...
  return [
    ...
    `2. git fetch origin ${baseBranch}:refs/remotes/origin/${baseBranch}（保存先を明示した refspec）で base を取得する。この base fetch の終了コードを必ず確認し、非ゼロ終了（通信・認証・refspec エラー等）の場合は merge も push も行わず pushed: false・commitFailed: true・summary に「base fetch 失敗」（エラー内容の要旨を添える）を書いて返す（fail-closed）。fetch 成功後、${baseMergeInstruction(baseBranch)} 分岐 (a) が Already up to date の場合は base 取り込み済みで積むものが無いため push せず pushed: false・commitFailed なし・summary に「Already up to date（GitHub 側の mergeable 算出待ちの可能性）」と書いて返す（次ラウンドの monitor が再確認する）。分岐 (a) がクリーンマージだった場合、分岐 (b) の解消（コンフリクトした submodule パスは base 側 — origin/${baseBranch} — が記録するコミットへポインタを合わせる: \`git checkout origin/${baseBranch} -- <path>\` の後 \`git add <path>\`。解消後は fmt / lint / build / test 等 PR 由来の未信頼コードを実行しない（任意コード実行→GitHub 認証情報奪取につながる。PR #443 P0）。妥当性確認は git diff --check の conflict marker（<<<<<<< 等）残存確認（0 件）のみに限定し、--no-verify 禁止のままコミットする。検証は push 後の既存 CI に委ねる）のいずれも完了した場合は手順 3 へ進む。分岐 (b) の解消不能・分岐 (c) の hook 拒否で返す場合は pushed: false・commitFailed: true・summary に上記理由を書いて返す。`,
