...
// 再発防止のため (b) は false で内容編集を禁じる（submodule 例外・他 commitFailed）。
function baseMergeInstruction(base, runVerification = true) {
  ...
  const resolveBranch = runVerification
    ...
    : `コンフリクト全パス（diff --name-only --diff-filter=U）が submodule ポインタ（gitlink。mode 160000）差分のみか判定する。通常ファイルの内容コンフリクトを含めば内容編集の権限を持たない（build/lint/test 不可のため）。commit せず abort し、pushed: false・commitFailed: true・summary「base コンフリクト解消不能（内容編集の権限なし。fix/implement へ委譲要）」を返す。submodule ポインタのみなら origin/${base} へ git checkout origin/${base} -- <path>、git add <path> で合わせ git commit --no-edit でコミットする${commitTail}`
...
function baseMergePrompt(item, impl, expectedRepo) {
  ...
  return [
    ...
    '手順:',
    `0. 本エージェントは隔離された git worktree 内で動作する。他のどの操作よりも先に \`git remote get-url origin\` の出力を取得し、末尾の改行・\`.git\` を除去したうえで SSH 形式（\`git@<host>:<owner>/<repo>\`・\`ssh://git@<host>/<owner>/<repo>\`）・HTTPS 形式（\`https://<host>/<owner>/<repo>\`）のいずれも host 部分と \`<owner>/<repo>\` 部分を別々に抽出する。まず host を ASCII 英大文字を英小文字へ変換したうえで許可ホスト "${ALLOWED_REMOTE_HOST}" と完全一致することを確認する（host が一致しない場合は owner/repo の比較へ進まず不一致として扱う。host を照合せず owner/repo だけを比較すると、期待リポジトリと同じ owner/repo を持つ別ホスト — 例: \`git@evil.example:${expectedRepoLiteral}.git\` — を誤って一致とみなし、別ホストへ fetch / push しうるため。PR #443 codex P0）。host が一致した場合に限り、owner/repo 部分を ASCII 英大文字を英小文字へ変換したうえで（GitHub の owner/repo は case-insensitive）、期待リポジトリ ${expectedRepoLiteral}（同じ規則で小文字化済み。空文字は未確定を意味し常に不一致として扱う）と完全一致することを確認する（小文字化は比較にのみ使う）。host と owner/repo の両方が一致した場合に限り続けて \`gh pr view ${impl.prNumber} --json number,headRefName\` を実行し、number が ${impl.prNumber}・headRefName が "${branch}"（ホスト確定済みブランチ名）と一致することを確認する（Issue タイトル等の未信頼テキストは参照しない。remote 一致確認の代替ではなく追加チェック）。remote の host が "${ALLOWED_REMOTE_HOST}" と不一致／owner-repo が期待リポジトリと不一致／PR を解決できない／number または headRefName が不一致のいずれか（= 別ホスト・別リポ worktree への誤配置・対象 PR の取り違え・ホスト側期待値未確定）なら、git fetch / checkout / merge / push を含む後続を一切実行せず routingError: true・pushed: false・summary に「worktree routing error: remote の host/owner-repo=<正規化後の値> が期待リポジトリ ${ALLOWED_REMOTE_HOST}/${expectedRepoLiteral} と不一致」を書いて即終了する（隔離契約違反のため）。`,
