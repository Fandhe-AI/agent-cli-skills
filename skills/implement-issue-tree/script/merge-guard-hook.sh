...
#        トークンは対象外）
#
#   ブレース展開はデコード用プリミティブとして raw 段階で deny する（Bugbot High 2件目）:
#   当初はブレース `{ }` と `,` の文字削除で `m{erge,}` → `merge` の復元を試みたが、
#   `m{e,}rge`（実行時展開は `merge` だが文字削除では `me` `rge` に分かれる）のように展開の
#   組み合わせ論を文字削除では追えないことが指摘された。ブレース展開が成立するには、ブレース
#   の最内グループに未クォートの `,` または `..` が必ず必要という構文的性質があるため、
#   ANSI-C クォート・IFS 展開と同じく「構文の存在自体」を raw コマンド段階で検知して deny
#   する（`{...,...}` / `{...  ..  ...}` を含む brace leaf の存在検知。クォート内かどうかは
#   追跡しないため、jq の object 構築（`{state, mergeCommit}`）や GraphQL の入力オブジェクト
#   （`{a:1,b:2}`）のようなリテラルも過検知で deny されるが、これは意図した over-deny —
#   subagent はコマンドを言い換えれば（jq なら `{state: .state}` を改行区切りにする等）
#   回避できる。glob の範囲・否定形（`[m-m]erge`・`[!x]` 等）は文字削除でも存在検知でも
#   完全には追えない残存リスクとして受容する — 正規の grep/sed 引数の文字クラスと構文上
#   区別できず、存在検知は正当コマンドの大量 over-deny になるため）。
...
# ブレース展開（`m{erge,}`・`m{e,}rge`・`{a..z}` 等）は実行時にのみ語が復元され、文字削除では
# 組み合わせ展開を追えないため、構文の存在自体を deny する。展開が成立する brace leaf
# （内側にブレースを含まないグループ）は未クォートの `,` か `..` を必ず含むという性質を使い、
# その形を検知する。クォート状態は追跡しないため jq・GraphQL のリテラル（`{a,b}` 等）も
