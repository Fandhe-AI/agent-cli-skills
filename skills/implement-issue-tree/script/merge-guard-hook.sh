...
#     1. glob ブラケット（[ ]）を除去する（`[m]erge` のように glob 展開で実行時に復元される
#        単純スペリングを文字レベルで復元する best-effort。ブレース展開はここではなく raw 段階の
#        デコード用プリミティブ検知で deny する — 後述）
#     2. シェルメタ文字（; & | < > ( ) { } `（バッククォート）・$ * ? , と改行）を無条件に
#        空白へ置換してトークン境界を確定させる（over-deny 設計。`merge;echo` は `merge`
#        `echo` の2トークンに分かれ "merge" として検出される）
...
#        トークンは対象外）
#
#   ブレース展開はデコード用プリミティブとして raw 段階で deny する（Bugbot High 2・3件目）:
#   当初はブレース `{ }` と `,` の文字削除で `m{erge,}` → `merge` の復元を試みたが、
#   `m{e,}rge`（実行時展開は `merge` だが文字削除では `me` `rge` に分かれる）のように展開の
#   組み合わせ論を文字削除では追えないことが指摘された。ブレース展開が 1 行内で成立するには、
#   その行の**最初の `{` と最後の `}` の間に**未クォートの `,` または `..` が必ず存在する
#   （ネストの有無に関わらず外側グループの区切りがこの範囲に現れる）という構文的不変条件を
#   使い、ANSI-C クォート・IFS 展開と同じく「構文の存在自体」を raw コマンド段階で貪欲一致
#   `\{.*(,|\.\.).*\}` で検知して deny する（当初は `[^{}]*` で brace leaf のみを見たが、
#   `m{e,{}}rge` のように区切り `,` が最内の非展開ブレース `{}` の外側にあるネスト形を
#   取りこぼしたため貪欲一致へ変更した）。クォート内かどうかは追跡しないため、jq の object 構築
#   （`{state, mergeCommit}`）や GraphQL の入力オブジェクト（`{a:1,b:2}`）のようなリテラルも
#   過検知で deny されるが、これは意図した over-deny — subagent はコマンドを言い換えれば
#   （jq なら `{state: .state}` を改行区切りにする等）回避できる。glob の範囲・否定形
#   （`[m-m]erge`・`[!x]` 等）は文字削除でも存在検知でも完全には追えない残存リスクとして
#   受容する — 正規の grep/sed 引数の文字クラスと構文上区別できず、存在検知は正当コマンドの
#   大量 over-deny になるため）。
...
# ブレース展開（`m{erge,}`・`m{e,}rge`・`m{e,{}}rge`（ネスト）・`{a..z}` 等）は実行時にのみ
# 語が復元され、文字削除では組み合わせ展開を追えないため、構文の存在自体を deny する。
# bash のブレース展開が 1 行内で成立するには、その行の**最初の `{` と最後の `}` の間に**
# 未クォートの `,` か `..` が必ず存在する（ネストの有無・深さに関わらず、外側グループの区切りが
# この範囲に現れる）。この不変条件を貪欲一致 `\{.*(,|\.\.).*\}` で検知する（当初は
# `[^{}]*` で brace leaf のみを見たが、`m{e,{}}rge` のように区切り `,` が最内の非展開ブレース
# `{}` の外側にあるネスト形を取りこぼしたため、貪欲一致へ変更した — Bugbot High 3件目）。
# grep は行単位で処理し `.` は改行に一致しないが、ブレース展開自体が改行をまたげないため
# 行単位一致で漏れはない。クォート状態は追跡しないため jq・GraphQL のリテラル（`{a,b}` 等）も
# 過検知で deny されるが、意図した over-deny（ファイル冒頭コメント参照）。
if printf '%s' "$cmd" | grep -qE '\{.*(,|\.\.).*\}'; then
  deny "ブレース展開構文（{...,...} / {a..z}）を含むコマンドは deny（展開で難読化されたマージ経路を防ぐ best-effort。jq や GraphQL のリテラルが過検知される場合はカンマを使わない形へ言い換えること。fail-closed）"
fi
...
# 合成順序」参照）:
#   (1) glob ブラケット（[ ]）を除去（`[m]erge` 等、実行時 glob 展開で復元される単純
#       スペリングの文字レベル復元。best-effort。ブレース展開は raw 段階の存在検知で deny 済み）
#   (2) シェルメタ文字（; & | < > ( ) { } `（バッククォート）・$ * ? ,）を無条件に空白へ
#       置換し、制御演算子が空白なしで連結された形（`merge;echo`・`merge&&true`・
#       `/usr/bin/gh&&true` 等）をトークン境界として分離する（over-deny 設計）
#   (3) 連続空白の圧縮
