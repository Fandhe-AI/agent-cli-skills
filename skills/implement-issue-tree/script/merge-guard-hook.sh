...
# （Issue #205）--------------------------------------------------------------------------
# 正規化済みコマンド全体（norm）が「gh pr merge <整数> --auto --squash」の正規形に完全一致
# する場合に限り deny をスキップする。判定は norm 全体の**完全一致**（^ ... $）で行い、
# 部分一致は使わない: 本 hook はコマンド区切り（; & | && ||）によるセグメント分割を採用せず
# norm を単位として判定する設計（下の判定ポリシーコメント参照）のため、コマンド連結
# （`gh pr merge 1 --auto --squash; rm -rf /` 等）や追加フラグ（`--admin` 等）・フラグ順
# 変更を含む形は完全一致に失敗し、通常どおり下の deny へ落ちる。
...
# （prmerge）も検出する。上の carve-out に完全一致しない限り、allow 経路はない。
