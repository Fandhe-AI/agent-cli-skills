...
function monitorPrompt(item, impl, externalApps, externalChecksConfirmed) {
  ...
  if (!externalChecksConfirmed) {
    ...
    step4Lines = [
      ...
      `   b. check-run に結論が出ていない（queued / in_progress）ものが残っている場合は Bugbot が実行中のため、催促せず手順 a から再確認する（needs-fix にはしない）。待機は手順 4 に入った時刻を起点とした通算 10 分を上限とし、通算 10 分を超えても未完了のままなら再確認を打ち切って state: blocked / blockedReason: "quality" を返して終了する（外部サービスがハングした場合に監視が終端しなくなるため。summary には「HEAD sha <sha> に対する cursor の check-run が通算 <実測> 分経過しても未完了」と状態別件数を書く。完了後の再実行で monitoring 再開により継続する）。結論が success / neutral / skipped 以外（failure / cancelled / timed_out）のものが 1 件でもあれば state: needs-fix とし、summary に状態別件数を書く。`,
