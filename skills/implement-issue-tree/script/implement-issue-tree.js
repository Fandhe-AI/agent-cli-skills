...
function monitorPrompt(item, impl) {
  return [
    ...
    '2. 全チェック完了後の CI 判定:',
    '   - 失敗チェックは gh run view --log-failed 等で原因を特定し state: needs-fix。summary に修正に必要な情報をすべて書く。変更と無関係な flaky と明確に判断できる場合に限り 1 回だけ gh run rerun <run-id> --failed で再実行して再監視する。再発した場合や変更起因の場合は state: needs-fix。',
    `   - Cursor Bugbot を利用しているリポジトリの場合: Bugbot の開始には時間がかかるため即座に催促しない。HEAD コミットの push から 1 分以上経過しても Bugbot チェックが開始していない場合のみ、HEAD push 以降に「@cursor review」コメントが未投稿であることを確認したうえで gh pr comment ${impl.prNumber} --body "@cursor review" を 1 回だけ投稿し、開始を最大 5 分待つ。投稿しても開始しない場合は再投稿せず Bugbot レビューなしとして扱い先へ進む（マージをブロックしない）。`,
    `   - Bugbot チェックの完了後、gh api repos/{owner}/{repo}/pulls/${impl.prNumber}/reviews で cursor[bot] の最新レビュー（PR の HEAD コミットに対するもの）を確認し、新規バグ指摘があればチェックが pass でも state: needs-fix とし指摘全文を summary に含める。過去コミットへの指摘で対応するレビュースレッドが resolved 済みのものは needs-fix の根拠にしない（修正済み指摘の再検出による偽 needs-fix を防ぐ）。`,
    '   - マージコンフリクトがあれば state: needs-fix とし、summary にコンフリクト解消が必要と書く。',
    '3. CI が全 green かつ Bugbot 指摘なしの場合、GraphQL API でレビュースレッドの全件を確認する（100 件超はページネーション必須）:',
    ...
    `4. CI 全 green・Bugbot 指摘なし・未解決レビューコメントなしなら gh pr merge ${impl.prNumber} --squash --delete-branch でマージする。`,
    `5. マージ後、gh issue view ${item.number} --json state でクローズを確認し、open のままなら gh issue close ${item.number} する。他のイシューが並列実行中のため、working copy のブランチ切り替えや git pull は行わない。`,
