...
function implementPrompt(item) {
  ...
  return [
    ...
    COMMON,
    '手順:',
    `0. worktree routing ガード（他のどの gh / git 操作よりも先に、最初に必ず実行する）: \`git remote get-url origin\` でカレント worktree の remote を確認し、\`gh issue view ${item.number} --json number,title\` で取得した title が「${title}」と一致することを確認する。remote が想定と異なる / issue が解決できない / title が不一致（= submodule 等の別リポ worktree に誤配置された状態。番号の存在だけでは別リポの同番号 issue を誤認するため title 一致まで要求する）のいずれかなら、後続（手順 0b の gh pr list・手順 2 の git fetch を含む一切の操作）を実行せず、即 prNumber: 0 と「worktree routing error: remote=<URL> でイシュー #${item.number}「${title}」を解決できず誤配置。実装リポの worktree への再配置が必要」を理由として返す。手動で別ディレクトリへ移動して作業しないこと（隔離契約違反・他エージェント干渉のため）。`,
