...
**auto-merge のサーバー側委譲（`sample/auto-merge.yml`）**: auto-merge を使いたい消費リポジトリは、同梱サンプル `sample/auto-merge.yml` を `.github/workflows/auto-merge.yml` として配置する。この workflow は `pull_request`（`opened` / `reopened` / `ready_for_review`）をトリガーに、provenance 3 条件の **AND** が成立する PR のみ `GITHUB_TOKEN` で `gh pr merge --auto --squash` を実行して GitHub ネイティブ auto-merge を有効化する: (1) **PR author がこのスキルの PR 作成専用 automation identity（bot / machine user。サンプル内の `REPLACE_WITH_AUTOMATION_BOT_LOGIN` を `your-automation-bot[bot]` 等へ置換）に完全一致**すること。**人間の個人アカウント（リポジトリ owner 含む）の指定は禁止**（その人物が手作業で作る通常 PR まで arm 対象になるため。専用 identity が未整備なら workflow を配置せず人間マージ運用に留める）。(2) head ブランチがこのスキルの命名規約 `<type>/<N>-<short-name>` にアンカー付き正規表現で**厳密一致**すること（`if:` の type prefix 判定は早期絞り込みで、確定判定は run 内の正規表現）。(3) PR 本文にこのスキルの PR Create フェーズが必ず書き込む生成物マーカー **`Closes #<N>`（N はブランチ名のイシュー番号と同一）が行として存在**すること。マージ可否の判定は arm ではなく **branch protection / ruleset の required checks・必須レビュー等（サーバー側）**が担う。permissions は `contents: write` + `pull-requests: write` の最小構成で、PR タイトル・本文等の未信頼テキストは run スクリプトへ一切展開しない（シェルが参照するのは PR 番号・リポジトリ名・env 経由でクォートしたブランチ名のみ。本文マーカーは `gh pr view --json body --jq` の test() で真偽値だけを取り出して判定する）。この方式では arm の実行主体がエージェントと権限・実行環境を共有しない GitHub Actions であり、クライアント側の subagent には arm 経路が存在しないため、PR #206 の認可欠陥は構造的に発生しない。ランは従来どおり PR をマージ可能状態（`blocked`）まで進めて停止し、監視中にサーバー側 auto-merge によって PR が MERGED になった場合は monitor の手順 1 が検出して already-merged 経路で正常完了する。
...
サーバー側 auto-merge 運用ではさらに **repo 設定で auto-merge を許可**する（Settings → General → Allow auto-merge）。
