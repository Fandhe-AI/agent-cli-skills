...
このスキルはネットワーク越しの GitHub 操作（`git fetch` / `git push` / PR 作成・マージ）を必須とする。該当コマンドはコマンド単位で sandbox 無効にして実行する（`docs/sandbox-tls.md` 参照。本スキル自身も同方針で `gh` / `git fetch` / `git push` をコマンド単位で sandbox 無効にして実行する）。ネットワーク遮断を解除できない環境では実行できない。
