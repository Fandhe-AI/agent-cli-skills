...
各サブイシューの実装過程で、対応すべきだが現スコープ外と判断した事項（未対応の改善・別機能・技術的負債・後続作業）が発生した場合は、放置せず必ず追跡する。
...
2. **記録は自動・Issue 書き込みは事後承認に分離する**
   各 implement エージェントはヘッドレス自動実行のため承認を待てない。実装中に検出した out-of-scope 項目は、その場では Issue 操作を行わず PR 本文の「対象外（out-of-scope）」節と Step 5 の最終レポートに記録するだけにとどめる。実際の Issue 書き込み（既存 Issue へのコメント追加 or 新規起票。手順 3・4）は、最終レポート確認時にユーザー（またはオーケストレータ）が out-of-scope 項目・既存 Issue の有無・対応案を確認し、**承認のうえで実行する**（確認なしに Issue 操作をしない）。
...
5. **PR 本文に記録する（個別エージェントは最終レポートに書き込まない）**
   各 implement エージェントが書けるのは自分の PR 本文のみ。Step 5 の最終レポートはツリー全体の実行後にオーケストレータが生成するため、個別エージェントは書き込めない。実装中の out-of-scope は対象 Issue の PR 本文の「対象外（out-of-scope）」節に「対象外とした項目」と対応案として記録する。最終レポート確認時に、ユーザー（またはオーケストレータ）が merged 各 PR 本文の当該節を集約し、Issue 化（手順 3・4）の承認・実行を行う。切り出し先 Issue 番号は承認後の起票で確定するため、記録時点では 'TBD' とする。
