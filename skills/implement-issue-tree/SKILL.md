...
```bash
# 1. 全子イシューが closed か確認（100 件超は page=2 以降も取得）
gh api "repos/{owner}/{repo}/issues/<parent>/sub_issues?per_page=100" --jq '.[].state'
...
```bash
# 親イシューの直下サブイシューが全て closed か確認（100 件超は page=2 以降も取得）
gh api "repos/{owner}/{repo}/issues/<parent>/sub_issues?per_page=100" \
  --jq '.[] | {number: .number, state: .state, title: .title}'

# 孫まで含む全サブイシューの状態確認（再帰が必要な場合は各 Phase 親でも実行）
gh api "repos/{owner}/{repo}/issues/<phase-parent>/sub_issues?per_page=100" \
