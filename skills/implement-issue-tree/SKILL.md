...
```json
{
  ...
  "args": {
    ...
    "maxResidualWorktrees": "<残置 worktree 総数の上限（0 以上の整数。省略時 100、0 で上限なし）>"
  }
}
```
