...
- **precheck（ラン単位・1 回）**: PR 作成前に、未信頼テキストを一切読まない読み取り専用エージェントが `gh api repos/{owner}/{repo} --jq '.allow_auto_merge'`（repo の auto-merge 許可）と base ブランチの required status checks 件数（ruleset・classic の両方）を確認する。host はこの結果を厳密再検証し、`autoMergeAllowed === true` かつ required checks 合計 `>= 1` のときのみ `autoMergeArmable = true` とする。取得不能・型不正・条件未達はすべて `false`（fail-closed）で、その場合は現行どおり blocked で停止して人間がマージする。
