...
description: >
  ...
  per-issue 計画立案（Plan: opus）→実装（Implement: sonnet）の分業。独立 Review フェーズ（マージ前ゲート、Low 含む指摘で修正ループ）。
...
### Step 4: ローカル diff を独立レビューする（Review）

Implement 完了後・Merge 前に、worktree 隔離で独立 Review エージェントを起動してローカル diff をレビューする。Review エージェントは**修正を行わず判定のみ**を担う。

レビュー条件:
- `git checkout --detach origin/<branch>` で対象ブランチを取得する
- `implement-review` スキルで品質・OWASP セキュリティをレビューする
- **Low（要改善）含む指摘が 1 件でも `needs-fix`**。指摘なしなら `ok`

`ok` の場合は Merge ステップへ進む。`needs-fix` の場合は fix エージェントで修正し再レビューする。**Review は最大 3 回**実施し、最終回（残り 0 回）の `needs-fix` では再レビューできないため fix を行わず収束失敗とする（修正後に必ず再レビューする原則を守るため。fix は実質最大 2 回）。3 回で収束しない場合は `blocked` として記録して次のイシューへ進む。
...
|--------|------|------------|
| `pending` | 未着手 | 最初から実行 |
| `planning` | 計画立案中（中断） | 最初から実行（Plan→Implement から再実行）。既存 open PR を再利用するため重複 PR は作成されない |
| `implementing` | 実装中（中断） | 最初から実行（Plan→Implement から再実行）。既存 open PR を再利用するため重複 PR は作成されない |
| `reviewing` | レビュー中（中断） | 最初から実行（Plan→Implement から再実行）。既存 open PR を再利用するため重複 PR は作成されない |
...
`planning` / `reviewing` からの再開は通常の Plan→Implement から再実行する。impl エージェントは着手時に既存 open PR を再利用するため重複 PR は作成されない。
