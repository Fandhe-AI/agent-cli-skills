...
| `autoMerge` | 任意 | `false` | **`true` + `externalChecks` 明示（確定）の opt-in ランでクライアント側 squash merge を実行する**（「クライアント側自動マージの設計」節参照。マージは merge-exec の自己取得再検証（HEAD sha・checks・スレッド・外部チェック）+ G0（ベースブランチの required status checks サーバー側強制の実測。確認できなければ `server-enforcement-missing` で `blocked` 終端）+ `--match-head-commit` + merge-verify の独立確認を経る。monitor の出力はマージ経路の入力に使われない）。既定 `false`・`externalChecks` 未確定時は従来どおりマージせず、PR はマージ可能状態の `blocked` で停止する（実装・push 前 Review・PR 作成・CI 監視・fix ループは値によらず自動で進む）。opt-in を使わない場合、auto-merge はサーバー側 workflow（upstream の `docs/implement-issue-tree/auto-merge-sample.yml`）+ branch protection への委譲、または GitHub 上での人間マージで行う（対象ブランチに branch protection を設定することを推奨）。注意: merge-guard hook 導入リポでは subagent の `gh pr merge` が deny されるため opt-in マージと hook は併用できない。boolean 以外はエラーで停止（誤記を黙って読み替えない） |
...
3. **G0 ゲート = サーバー側強制の実測**: merge-exec はマージ実行前に、ベースブランチへ required status checks のサーバー側強制（ruleset / classic branch protection）が構成されていることを件数のみの API 出力で実測確認し、確認できなければ `server-enforcement-missing` で辞退して `blocked` 終端する（fail-closed）。**マージ可否の実強制は GitHub の branch protection であり**、エージェントの判定が同時に誤っていても required checks が green でない・未解決スレッドが残る・承認が足りない PR のマージはサーバーが拒否する。
