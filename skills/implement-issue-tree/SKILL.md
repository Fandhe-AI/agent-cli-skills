...
サーバー側 auto-merge 運用ではさらに **repo 設定で auto-merge を許可**する（Settings → General → Allow auto-merge）。なお `sample/auto-merge.yml` は上記のうち **required checks >= 1・非 author 必須承認 >= 1・dismiss stale reviews の 3 点を arm 前に API で実測検証し、未構成なら arm しない（fail-closed）**。classic branch protection の読み取り API は管理者権限を要求し workflow の `GITHUB_TOKEN` では読めないことがあるため、**ruleset での構成を推奨**する（実効ルール API は読み取り権限で取得できる）。
