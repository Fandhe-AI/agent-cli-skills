...
out-of-scope 項目を検出した場合は本レポートに「対象外とした項目」と切り出し先 Issue 番号を含める（後述の「実装対象外（out-of-scope）の扱い」を参照）。out-of-scope の収集はレビュー中（Step 2〜3）に行う。
...
5. **レビューレポートに明記する**
   out-of-scope 項目はレビュー中（Step 2〜3）に収集し Step 4 のレポートに含める。Issue への書き込み操作は承認後に行う。レビューレポートに「対象外とした項目」と「切り出し先 Issue 番号」を記載する。
