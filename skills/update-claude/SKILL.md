...
symlink は `readlink` で現在のターゲットを検証し、期待ターゲットと異なる場合（stale）・参照先が消失している場合（dangling）は張り替える。実体ファイル（非 symlink）は上書きしない。対象リポに `dotclaude-via-temp` ルールがある場合はそのルールに従い `_/dotclaude/` 経由で配置する（ない場合は `ln -s` 直接作成でも可）。
